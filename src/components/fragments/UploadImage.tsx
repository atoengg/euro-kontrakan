...
export const UploadImage = ({ onUploadComplete }: UploadImageProps) => {
    ...
    const handleUploadComplete = useCallback(async (url: string) => {
        ...
        try {
            const response = await fetch('/api/upload-image', {
                ...
                body: JSON.stringify({
                    url, // URL gambar dari UploadThing
                    uploadedBy: user.uid, // ID user saat ini
                }),
            });
            ...
            console.error("Error saat upload image:", error);
                    ...
                    <UploadButton
                        ...
                        onClientUploadComplete={(res) => {
                            if (res && res.length > 0) {
                                ...
                                if (uploadedUrl) handleUploadComplete(uploadedUrl);
                            }
                        }}
                        onUploadError={(error) => {
                            console.error("Upload error:", error); // Debugging error
